...
from jsk_arc2017_common.srv import SaveJSON
from jsk_arc2017_common.srv import SaveJSONResponse
...
class JSONSaver(threading.Thread):
    ...
    def __init__(self):
        ...
        self.output_json_path = osp.join(output_dir, 'item_location_file.json')
    ...
    def _save(self, req):
        is_saved = self._save_json()
        return SaveJSONResponse(saved=is_saved)
    ...
    def _update_location(self, req):
        ...
        try:
            ...
            self.bin_contents[bin_].append(item)
        except Exception:
