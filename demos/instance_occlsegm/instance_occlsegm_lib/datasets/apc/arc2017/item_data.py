...
import instance_occlsegm_lib.func
...
class ItemData(object):
    ...
    def load_from_img_file(self, img_file):
        ...
        # crop
        y1, x1, y2, x2 = instance_occlsegm_lib.image.masks_to_bboxes([mask])[0]
        img = img[y1:y2, x1:x2]
        mask = mask[y1:y2, x1:x2]
        lbl_suc = lbl_suc[y1:y2, x1:x2]

        # resize
        img = instance_occlsegm_lib.image.resize(img, size=self._img_size)
        mask = instance_occlsegm_lib.image.resize(
            mask.astype(np.int32),
            size=self._img_size,
            interpolation=cv2.INTER_NEAREST,
        ).astype(bool)
        lbl_suc = instance_occlsegm_lib.image.resize(
...
class ItemDataARC2017(ItemData):
    ...
    def download(self):
        ...
        postprocess = functools.partial(
            instance_occlsegm_lib.data.extract, path, to=osp.dirname(path),
        )
...
def _directory_to_hash(data_dir, *args, **kwargs):
    return instance_occlsegm_lib.data.directory_to_hash(data_dir)
...
def _load_item_data(args):
    ...
    for img_file in glob.glob(osp.join(obj_dir, '*.png')):
        ...
        obj_data.append({
            ...
            'lbl': instance_occlsegm_lib.image.mask_to_lbl(mask, obj_id),
        })

    return obj_data


@instance_occlsegm_lib.func.cache(key=_directory_to_hash)
