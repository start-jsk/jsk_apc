...
# XXX: Very similar to MaskRCNNTrainChain.
class MaskRCNNPanopticTrainChain(chainer.Chain):
    ...
    def __call__(
        ...
        * :math:`R` is the number of bounding boxes per image.
        ...
        if self._notrain == 'ins':
            ...
            sample_roi_indices = self.xp.concatenate(
                sample_roi_indices, axis=0
            )
        ...
        if self._notrain == 'pix':
            pass
        else:
            scores_vis, scores_occ = self.mask_rcnn.head_pix(
                features, img_size,
            )
            pix_vis_loss = F.softmax_cross_entropy(scores_vis, lbls_vis)
            pix_occ_loss = F.sigmoid_cross_entropy(scores_occ, lbls_occ)
            loss += self._pix_loss_scale * (pix_vis_loss + pix_occ_loss)
            chainer.reporter.report({
                'pix_vis_loss': pix_vis_loss,
                'pix_occ_loss': pix_occ_loss,
            }, self)
