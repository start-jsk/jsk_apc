...
def calc_semantic_segmentation_fwiou(confusion):
    t = confusion.sum(axis=1)
    iou_denominator = (t + confusion.sum(axis=0) -
                       np.diag(confusion))
    fwiou = ((t * np.diag(confusion)) / iou_denominator).sum(axis=0) / t.sum()
    return fwiou
...
def eval_semantic_segmentation(pred_labels, gt_labels):
    ...
    * :math:`\\text{Mean Class Accuracy} = \\frac{1}{k} \
    ...
    fwiou = calc_semantic_segmentation_fwiou(confusion)
