...
class DualarmGraspFCN32s(chainer.Chain):
    ...
    def __call__(self, x, label=None, single_grasp=None, dual_grasp=None):
        ...
        loss += self.alpha_dual * dual_grasp_loss
        return loss

    def init_from_vgg16(self, vgg16):
        for lyr in self.children():
            if lyr.name.startswith('conv'):
                l1 = getattr(vgg16, lyr.name)
                l2 = getattr(self, lyr.name)
                assert l1.W.shape == l2.W.shape
                assert l1.b.shape == l2.b.shape
                l2.W.data[...] = l1.W.data[...]
                l2.b.data[...] = l1.b.data[...]
            elif lyr.name in ['fc6', 'fc7']:
                l1 = getattr(vgg16, lyr.name)
                l2 = getattr(self, lyr.name)
