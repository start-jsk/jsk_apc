...
def check_param(param_name, expected, must=False):
    ...
    if target_param == expected:
        success_print("Parameter " + param_name + " is " + expected)
    else:
        fail_print("Parameter " + param_name + " is " + target_param + ". Doesn't match with exepcted value : " + expected)
...
def check_node(target_node_name, needed, sub_success="", sub_fail=""):
    ...
    if target_node_name in nodes:
        if needed:
            ...
            if sub_success:
                print OK_GREEN+"    "+sub_success,ENDC
        else:
...
if __name__ == "__main__":
    ...
    check_node("/baxter_joint_trajectory", False, sub_fail="baxter.launch seems to be launched somewhere")
    check_node("/robot_state_publisher", False, sub_fail="challenge.launch seems to be launched somewhere")
    check_node("/kinect2_bridge", False, sub_fail="baxter.launch seems to be launched somewhere")
    check_node("/kinect2_points_xyzrgb_highres", False, sub_fail="baxter.launch seems to be launched somewhere")

    index_print("== Check TOPICS ==")
    check_topic("/robot/state", True, 3)

    index_print("== Check PARAMETERS ==")
    # check_param("/hogehoge", "test")
    # check_param("/aginika", "aginika")
